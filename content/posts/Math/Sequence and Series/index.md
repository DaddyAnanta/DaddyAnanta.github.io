...
- **Jumlah Total HP:** 
  - HP tidak memiliki rumus pasti. Perhitungannya menggunakan estimasi [logaritma](https://pepuru.com/posts/math/exponents-and-logarithms/): $S_n \approx \frac{1}{d} \ln\left(\frac{2a + (2n - 1)d}{2a - d}\right)$.
...
| **Aritmatika-Geometri (AGP)**    | **Urutan ke-$n$:** $[a + (n - 1)d]r^{n-1}$<br><br>**Total hingga urutan ke-$n$:** Dihitung dengan menyederhanakan perhitungan bentuk [aljabarnya](https://pepuru.com/posts/math/algebraic-identity/). | Rumus tak hingganya adalah $\frac{a}{1 - r} + \frac{dr}{(1 - r)^2}$. Berlaku jika pengalinya di bawah $1$, lihat [tahap pembuktiannya](https://proofwiki.org/wiki/Sum_of_Infinite_Arithmetic-Geometric_Sequence). |
